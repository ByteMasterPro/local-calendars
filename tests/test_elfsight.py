...
from brewcal.model import Brewery
...
def test_parses_every_fixture_event():
    events = elfsight.parse_events(SETTINGS, BREWERY)
...
def test_timed_event_is_local_tz_aware():
    ev = by_name(elfsight.parse_events(SETTINGS, BREWERY), "Honorfest")
    ...
    assert ev.location == BREWERY.location
...
def test_action_link_becomes_url_and_description_line():
    ev = by_name(elfsight.parse_events(SETTINGS, BREWERY), "3rd Annual Classic Lite Beer Mile")
...
def test_zero_length_event_gets_default_duration():
    ev = by_name(elfsight.parse_events(SETTINGS, BREWERY), "Honorfest Beer Dinner")
    assert (ev.end - ev.start).total_seconds() == BREWERY.default_duration_minutes * 60


def test_all_day_event_uses_dates_with_exclusive_end():
    ev = by_name(elfsight.parse_events(SETTINGS, BREWERY), "Can Release Day")
    assert ev.all_day
...
def test_recurring_event_maps_to_rrule_exdate_and_named_location():
    ev = by_name(elfsight.parse_events(SETTINGS, BREWERY), "Trivia Night & Tacos")  # entity unescaped
...
def test_render_round_trips_and_has_bounded_vtimezone():
    events = elfsight.parse_events(SETTINGS, BREWERY)
    ics = ical.render(BREWERY, events)
...
def test_strip_volatile_makes_repeat_renders_equal():
    events = elfsight.parse_events(SETTINGS, BREWERY)
    a = ical.render(BREWERY, events, now=datetime(2026, 1, 1, tzinfo=timezone.utc))
    b = ical.render(BREWERY, events, now=datetime(2026, 6, 1, tzinfo=timezone.utc))
    assert a != b
